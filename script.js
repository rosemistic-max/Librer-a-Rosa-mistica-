...
function initMenuYBusqueda() {
  ...
  function ejecutarBusqueda() {
    ...
    // Despliegue seguro de Productos en Grid (Con optimización de imágenes al vuelo)
function filtrarYMostrarProductos(busqueda = '', categoria = 'Todos') {
  ...
  productosFiltrados.forEach(p => {
    ...
    let imagenOptimizada = p.imagen;
    if (imagenOptimizada.includes('ibb.co') && !imagenOptimizada.includes('i.ibb.co')) {
      imagenOptimizada = imagenOptimizada.replace('ibb.co', 'i.ibb.co');
    }
    
    card.innerHTML = `
      <div class="card-image-container">
        <!-- Usamos la imagen optimizada que pesa una fracción de la original -->
        <img src="${imagenOptimizada}" alt="${p.nombre}" loading="lazy">
      </div>
      <div class="card-body">
